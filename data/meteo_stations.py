"""Fetch Météo-France open data (daily temperatures by department)."""
...
# Direct .csv.gz URLs per department (data.gouv.fr)
CIBLES_METEO = {
...
def filtrer_donnees_meteo(chemin_gz: str | Path) -> pd.DataFrame:
    """Read a gzipped SYNOP CSV and keep useful columns since 2000-01-01."""
    df = pd.read_csv(
        chemin_gz,
        sep=";",
        low_memory=False,
...
def fetch_meteo_data(
    ...
    """Download and filter meteo observations for the given departments.

    Args:
        departments: Mapping dept code -> download URL. Defaults to CIBLES_METEO.
        raw_dir: Directory for temporary .csv.gz files. Defaults to a temp dir.

    Returns:
        Concatenated DataFrame with columns:
        NUM_POSTE, DATE, NOM_USUEL, TMIN, TMAX, TMEAN, LAT, LON, ALTI, TMED_TN_TX.
    """
    cibles = departments if departments is not None else CIBLES_METEO
    frames: list[pd.DataFrame] = []

    if raw_dir is None:
        tmp_ctx = tempfile.TemporaryDirectory(prefix="meteo_raw_")
        dossier = Path(tmp_ctx.name)
    else:
        tmp_ctx = None
        dossier = Path(raw_dir)
        dossier.mkdir(parents=True, exist_ok=True)

    try:
        for dept, url in cibles.items():
            ...
            chemin_gz = dossier / f"dep_{dept}.csv.gz"
            try:
                chemin_gz.write_bytes(response.content)
                frames.append(filtrer_donnees_meteo(chemin_gz))
                print(f"Département {dept} chargé et filtré.")
...
if __name__ == "__main__":
    destination = Path(__file__).resolve().parent / "raw_data"
    df = fetch_meteo_data(raw_dir=destination)
